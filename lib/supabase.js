...
export const signUp = async (email, password, userType = 'user', profileData = {}, locationData = {}) => {
  try {
    ...
    const profileRecord = {
      ...
      points: 0,
      is_verified: true, // Auto-verify new users for now
    ...
    // Insert profile into Supabase (renamed to avoid shadowing)
    const { data: profileRecordData, error: profileError } = await supabase
      .from('profiles')
    ...
    console.log('Profile created successfully:', profileRecordData);
...
export const signIn = async (email, password) => {
  try {
    ...
    if (!authData.user.email_confirmed_at) {
      console.log('User email not confirmed:', authData.user.email);
      return { 
        data: null, 
        error: { 
      ...
      if (profileError.code === 'PGRST116') {
        ...
        const { data: newProfile, error: createError } = await supabase
          ...
          .insert([{
            ...
            is_verified: true, // Auto-verify if they can sign in
            verified_at: new Date().toISOString(),
            points: 0,
            created_at: new Date().toISOString(),
            updated_at: new Date().toISOString()
        ...
        return { data: null, error: { message: 'Failed to load user profile' } };
      }
    }
